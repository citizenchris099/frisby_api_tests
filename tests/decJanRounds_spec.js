/**
 * Created by kenziegossett on 5/19/17.
 */
...
var taskJson = function (id, details) {
    return {
        ...
        "client": "uM5jSPfAJtAtezipe", "round": id, "isBlocked": false, "blockedReason": null,
...
var decemberJanuaryScenarioDetails = function (get) {
    var details = new Map();
    var hourlyStartDate = utility.now(0, "2018-12-31T00:00:00.000Z");
    var dailyStartDate = utility.now(0, "2018-12-29T00:00:00.000Z");
    var startDate = utility.now(0, "2018-12-01T00:00:00.000Z");
    var dueWithin = {"minutes" : 30};

    var hourlyTasksExpected = 25;
    var dailyTasksExpected = 4;
    var weeklyTasksExpected = 5;
    var bwTasksExpected = 3;
    var monthlyTasksExpected = 2;
    var quarterlyTasksExpected = 1;
    var saTasksExpected = 1;
    var annuallyTasksExpected = 1;

    var hourlyCreateExpect = utility.expectArray(hourlyTasksExpected, 'h', 0,"2018-12-31T00:00:00.000Z");
    var hourlyDueExpect = utility.dueDateArray(hourlyCreateExpect, 'h',1);
    var dailyCreateExpect = utility.expectArray(dailyTasksExpected, 'd', 0,"2018-12-29T00:00:00.000Z");
    var dailyDueExpect = utility.dueDateArray(dailyCreateExpect, 'h',1);
    var weeklyCreateExpect = utility.expectArray(weeklyTasksExpected, 'w', 0,"2018-12-01T00:00:00.000Z");
    var weeklyDueExpect = utility.dueDateArray(weeklyCreateExpect, 'h',1);
    var bwCreateExpect = utility.expectArray(bwTasksExpected, 'bw', 0,"2018-12-01T00:00:00.000Z");
    var bwDueExpect = utility.dueDateArray(bwCreateExpect, 'h',1);
    var monthlyCreateExpect = utility.expectArray(monthlyTasksExpected, 'M', 0,"2018-12-01T00:00:00.000Z");
    var monthlyDueExpect = utility.dueDateArray(monthlyCreateExpect, 'h',1);
    var quarterlyCreateExpect = utility.expectArray(quarterlyTasksExpected, 'Q', 0,"2018-12-01T00:00:00.000Z");
    var quarterlyDueExpect = utility.dueDateArray(quarterlyCreateExpect, 'h',1);
    var saCreateExpect = utility.expectArray(saTasksExpected, 'sa', 0,"2018-12-01T00:00:00.000Z");
    var saDueExpect = utility.dueDateArray(saCreateExpect, 'h',1);
    var annuallyCreateExpect = utility.expectArray(annuallyTasksExpected, 'y', 0,"2018-12-01T00:00:00.000Z");
...
describe('round december to january scenario', function () {
    it('can run the december to january rounds scenario', function () {
        utility.scenario('rounds', getArray, decemberJanuaryScenarioDetails, roundToPost, roundExpect, taskJson, utility.now(0,"2019-01-01T00:00:00.000Z"))
    });
});
